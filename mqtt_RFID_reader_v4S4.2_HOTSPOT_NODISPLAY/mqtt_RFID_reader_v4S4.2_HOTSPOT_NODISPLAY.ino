...
const unsigned long RFID_READ_INTERVAL = 300000;  // 10 menit dalam milidetik (10 * 60 * 1000)

unsigned long lastTunggurespon = 0;
const unsigned long TUNGGU_RESPON_SERVER = 5000;

unsigned long previousLEDmqtt = 0;
const unsigned long intervalLEDmqtt = 100;
const unsigned long intervalLEDmqtt2 = 500;
  ...
  if (digitalRead(SET_BTN) == HIGH || bootAP == true) {
    ...
    if (ssidNew != "") {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        unsigned long startAttemptTime = millis();  // Waktu awal mencoba koneksi MQTT
        ...
        while (!client.connected()) {
          if (client.connect("NodeMCUClient", mqtt_user, mqtt_password)) {
            ...
            // Indikasi kegagalan koneksi
            digitalWrite(LED_PIN, HIGH);
            buzzBasedOnMessage("404");
            digitalWrite(LED_PIN, LOW);

            // Periksa apakah waktu mencoba koneksi sudah melebihi 1 menit
            if (millis() - startAttemptTime >= 60000) {
              Serial.println("Gagal koneksi ke MQTT selama 1 menit. Restarting...");
