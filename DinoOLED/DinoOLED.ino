#include <U8g2lib.h>
#include <SPI.h>

// 'pngegg (1) (1)', 32x32px
const unsigned char epd_bitmap_pngegg__1___1_[] = {
  
};
// '8iEb4KGBT (1)', 32x32px
const unsigned char epd_bitmap_8iEb4KGBT__1_[] = {
  
};

U8G2_SSD1306_128X64_NONAME_F_SW_I2C u8g2(U8G2_R0, /* clock=*/SCL, /* data=*/SDA, /* reset=*/U8X8_PIN_NONE);

void setup(void) {
  u8g2.begin();

  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_luBIS08_tf);
  u8g2.drawStr(20, 10, "Hello Guys!");
  u8g2.drawStr(20, 30, "Hello Bos!");
  u8g2.drawStr(20, 50, "Hello Hai!");
  u8g2.sendBuffer();
  delay(2000);

  u8g2.clearBuffer();
  u8g2.drawStr(20, 10, "Halo Speedy!");
  u8g2.drawXBM(48, 16, 32, 32, epd_bitmap_8iEb4KGBT__1_);
  u8g2.sendBuffer();
  delay(2000);
}

void loop(void) {
  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(-16, 16, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(300);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(0, 32, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(500);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(16, 16, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(300);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(32, 32, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(500);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(48, 16, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(300);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(64, 32, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(500);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(80, 16, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(300);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(96, 32, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(500);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(112, 16, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(300);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.drawXBM(120, 32, 32, 32, epd_bitmap_pngegg__1___1_);
  u8g2.sendBuffer();
  delay(500);

  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "Halo Dyno!");
  u8g2.sendBuffer();
  delay(500);
}
