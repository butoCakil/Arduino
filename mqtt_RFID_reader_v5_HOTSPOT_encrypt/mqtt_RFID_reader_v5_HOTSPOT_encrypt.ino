...
String encryptionKey = "siap$boss";  // Kunci enkripsi yang sama dengan di server PHP
...
void setup() {
  ...
  String dataToEncrypt = "Hello, Server!";
  encryptAndSendData(dataToEncrypt);
  Serial.print("Encripsi data: ");
  Serial.println(en);
}
...
void encryptAndDisplay(String data) {
  AES aes;
  aes.set_key((const uint8_t*)encryptionKey, 16);  // Set the encryption key

  // Encrypt the data
  String encryptedData = aes.encrypt(data.c_str());

  // Display the encrypted data in Serial Monitor
  Serial.print("Data Asli: ");
  Serial.Serial
    println(data);
  Serial.Serial
    print("Data Terenkripsi: ");
