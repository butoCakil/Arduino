...
String aktuator;
int i, j;
unsigned long praMillis = 0;
unsigned long interval = 60 * 60 * 1000;
unsigned long prareboot = 60 * 1000;
int timer = 60;

byte bigdot[8] = {B00000, B00000, B01110, B11111, B11111, B01110, B00000, B00000};
byte arrow_blk_r[8] = {
  B01110, 
  B10111, 
  B10011, 
...
void setup() {
  ...
  lcd.createChar(3, bigdot);
  lcd.createChar(4, arrow_blk_r);
...
void reboot(){
  unsigned long mulaiulang = millis();

  if(mulaiulang - praMillis >= (interval - prareboot)){

    if((millis() % 1000 <= 24) && (millis() % 1000 >= 0)){
      timer--;
      ...
      if((timer/10) % 2 == 0){
        lcd.setCursor(1, 0);
        lcd.print("TAP utk Batalkn");
      } else {
        lcd.setCursor(1, 0);
        lcd.print(" Segera Restart!");
      }
      lcd.setCursor(0, 1);
      lcd.print("     00:");
  ...
  if(mulaiulang - praMillis >= interval){
    praMillis = millis();
    lcd.setCursor(0, 0);
    lcd.print("Hidupkan Ulang..");
    lcd.setCursor(0, 1);
    lcd.print(">> Restart......");
    delay(200);
    ESP.restart();
