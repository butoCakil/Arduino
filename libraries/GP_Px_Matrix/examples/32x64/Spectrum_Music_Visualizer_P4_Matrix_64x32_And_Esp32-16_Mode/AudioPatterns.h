...
class AudioPatterns : public Playlist {
  ...
  public:
    ...
    static const int PATTERN_COUNT = 16;
    ...
    void SpMuAudio16() {
      analyzerLinesByIntensity(64, 32);
    }
    ...
    void drawAudio1() {
      ...
      for (int band = 0; band < bandCount; band++) {
        ...
        uint16_t color1 = MyColor[(level * 3) % 26];
    ...
    void drawAudio2() {
      ...
      for (int band = 0; band < bandCount; band++) {
        ...
        if (band == 15) nextLevel = levels[0];
        uint16_t color1 = MyColor[(level * 3) % 26];
        uint16_t color2 = MyColor[(nextLevel * 3) % 26];
        float radians = radians(angle);
        int x1 = CENTRE_X + level * 2 * sin(radians);
        int y1 = CENTRE_Y + level * cos(radians);
        radians = radians(angle - degreesPerLine);
    ...
    void drawAudio11(int Wx, int Hy) {
      ...
      for (int i = 0; i < bandCount; i++) {
        ...
        int x1 = i * dx, y1 = y;
        int x2 = i * dx + dx, y2 = nextY;
        effects.fillTriangle(x1, y1, x2, y2, x1, 31, color1);
        effects.fillTriangle(x1, 31, x2, 31, x2, y2, color2);
      }
    }
    ...
    void drawAudio12(int Wx, int Hy) {
      ...
      for (int i = 0; i < bandCount; i++) {
        ...
        int x1 = i * dx, y1 = y;
        int x2 = i * dx + dx, y2 = nextY;
        effects.fillTriangle(CENTRE_X, 31, x1, y1, x2, y2, color1);
      }
    }
    ...
    void drawAudio13() {
      ...
      for (int band = 0; band < bandCount; band++) {
        ...
        effects.fillTriangle(x1, y1, x2, y2, x3, y3, color1);
        angle += degreesPerLine;
        if (angle > 90) angle -= degreesPerLine - 90;
      }
    }
    ...
    void drawAudio14() {
      ...
      for (int band = 0; band < bandCount; band++) {
        ...
        int x3 = CENTRE_X + nextLevel * 2 * sin(radians);
        int y3 = CENTRE_Y + nextLevel * cos(radians);
        effects.drawTriangle(x1, y1, x2, y2, x3, y3, color1);
        angle -= degreesPerLine;
      }
    }

    void drawAudio15(int Wx, int Hy) {
        int dx = Wx / bandCount;
        int dy = MATRIX_HEIGHT / Hy;
      for (int i = 0; i < bandCount; i++) {
        int level = levels[i] * 2;
        ...
        int x1 = i * dx, y1 = y;
        int x2 = i * dx + dx, y2 = nextY;
        effects.drawTriangle(x1, y1, x2, y2, x2, y1, color1);
      }
    }
